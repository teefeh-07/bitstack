...
function processCommits(branchData) {
    ...
    // git fetch to make sure main is at latest
    try { run(`git pull origin main`); } catch (e) { }
...
function main() {
    const plan = JSON.parse(fs.readFileSync('robust_plan.json', 'utf8'));
    ...
    console.log('\\nAll robust branches processed and merged!');
}
